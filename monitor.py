...
DESTINATARIOS = ["deusefielweb2001@gmail.com"]
...
def disparar_protocolo(motivo):
    ...
    msg['Subject'] = f"🚨 TESTE DE EMERGÊNCIA: PROTOCOLO GO-2025"

    corpo = f"""
    ESTE É UM TESTE DO SEU SISTEMA DE VIGILÂNCIA.
    
    O gatilho foi ativado por: {motivo}.
    A conexão entre o GitHub e o seu Gmail de disparo funcionou.
    ...
    try:
        # Usando SSL na porta 465 (mais seguro para scripts)
        server = smtplib.SMTP_SSL('smtp.gmail.com', 465)
        server.login(user, password)
        server.sendmail(user, DESTINATARIOS, msg.as_string())
        server.quit()
        print(f"✅ E-mail de teste enviado com sucesso para {DESTINATARIOS}")
    except Exception as e:
        print(f"❌ Falha técnica no disparo: {e}")

def verificar_integridade():
    # 1. Verifica sabotagem
    sha256 = hashlib.sha256(open(ARQUIVO_ALVO, 'rb').read()).hexdigest()
    if sha256 != HASH_ORIGINAL:
        disparar_protocolo("SIMULAÇÃO DE SABOTAGEM")
        return True
